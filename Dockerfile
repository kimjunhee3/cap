...
# Gunicorn으로 recent_back:app 실행
# (Railway는 PORT를 주입하므로 -b 0.0.0.0:${PORT})
CMD ["sh","-c","gunicorn recent_back:app -b 0.0.0.0:${PORT} --workers 1 --threads 2 --timeout 300 --log-level info"]
